...
import sleventextraction.SLMentionRecorder;
...
public class Ace {
    ...
    static SLEventTypeClassifier EventTypeClassifier;
    static SLMentionRecorder SCCRFTestRecorder = new SLMentionRecorder();
    static SLRegularization regulator;
     ...
     * outDirectory: the path of the directory in which APF files are to be
     * written                                              <BR>
     * parseDir: the path of the directory containing parse trees [optional]
     * <BR>
     * glarfDir: the path of the directory containing GLARF tuples [optional]
     * <BR>
    ...
    public static void main(String[] args) throws IOException {
        args = new String[4];
        args[0] = "D:\\Grade2_2\\MSRA\\RBET_release\\RBET_release\\props\\ace11chunker.properties";
        args[1] = "D:\\Grade2_3\\LDC2006D06\\Data\\LDC2006T06_Original\\data\\English\\nw\\Testfile.txt";
        args[2] = "D:\\Grade2_3\\LDC2006D06\\Data\\LDC2006T06_Original\\data\\English\\nw\\fp1\\";
        args[3] = "D:\\Grade2_3\\LDC2006D06\\Data\\LDC2006T06_Original\\data\\English\\trainout\\";
        initForFileIO(args);
        SLEventExtraction.ConstructWordVec();;
        EventTypeClassifier = new SLEventTypeClassifier();
        regulator = new SLRegularization();
        // open list of files
        BufferedReader reader = new BufferedReader(new FileReader(fileList));
        // debug = false ==> try to catch all exceptions and continue
        boolean debug = false;
        while ((currentDocPath = reader.readLine()) != null) {
            SLEventExtraction.parseFile("D:\\Grade2_3\\LDC2006D06\\Data\\LDC2006T06_Original\\data\\English\\nw\\fp1\\" + currentDocPath);
            if (debug) {
    ...
    /**
     * 'initForFileIO' provides the initialization component of the 'main'
     * method.
     */
    ...
    /**
     * initialize ACE: read property file and load all resources specified by
     * the properties file.
     */
    ...
    /**
     * if <CODE>paramName</CODE> is a numeric-valued parameter in the Jet
     * configuration file, returns that value as a <CODE>double</CODE>, else
    ...
    /**
     * process a single document to generate an Ace APF file, catching any
     ...
     *
     * @param currentDocPath The full path of the file containing the source
     * document
     */
     ...
     *
     * @param sourceId The source document id; typically the 'sourceFile', but
     * without any file extension. If no DOCID is specified within the document
     * text) as the base for the names of all entities, relations, events, etc.
     *
     * @param sourceFile The name of the source document. This is used to fill
     * the source file field of the AceDocument. If 'doc' is read from a file,
     * should be the file name, including extension, but not including directory
     * path.
     *
     * @param docPathBase The full path to the source document, with the file
     * extension removed; used to access corresponding parse and GLARF files.
     * Should be null if these files are not being used
     *
    ...
    /**
     * returns the document ID of Document <CODE>doc</CODE>, if found, else
     * returns <CODE>null</CODE>. It looks for <br>
     * the text of a <B>DOCID</B> annotation (as provided with ACE documents);
     * <br>
     * the text of a <B>DOCNO</B> annotation (as provided with TDT4 and TDT5
     * documents); <br>
     * the <B>DOCID</B> feature of a <B>document</B> annotation (for GALE ASR
     * transcripts); or <br>
     * the <B>id</B> feature of a <B>doc</B> annotation (for newer LDC
     * documents).
     */
     ...
     * appears to be capitalized as a title: if there are no words beginning
     * with a lower-case letter except for a small list of function words
     * (articles, possessive pronouns, prepositions, ...).
     */
    ...
    /**
     * create an AceEntity from <CODE>entity</CODE>. If the entity is not a
     * valid EDT type, nothing is written.
     */
    ...
    /**
     * write the information for <CODE>mention</CODE> with head
     * <CODE>head</CODE> in APF format.
     */
    ...
    /**
     * determine the mention type of a mention (NOMINAL, PRONOUN, or NAME) from
     * its <CODE>head</CODE>.
     */
    private static String mentionType(Annotation head, Annotation mention) {
        ...
        // 	return "PRE";
        // else
        if ("n".equals(cat) || "title".equals(cat) || "tv".equals(cat) || "v".equals(cat)) {
            return "NOMINAL";
        } else if ("pro".equals(cat) || "det" /*for possessives - his, its */.equals(cat) || "adj".equals(cat) || "ven".equals(cat) || "q".equals(cat)
                || "np" /* for headless np's */.equals(cat) || "wp".equals(cat) || "wp$".equals(cat)) {
            return "PRONOUN";
        } else // cat == "name"
    ...
    private static boolean isGeneric(Document doc, Annotation mention) {
        ...
        if (headC.get("cat") == "n") {
            ...
            if (ngHead.get("poss") != null || "poss".equals(det)) {
                return false;
            }
            if (ngHead.get("quant") != null || "q".equals(det)) {
                return false;
            }
    ...
    /**
     * tag the document <CODE>doc</CODE> for time expressions, and create (and
     * add to <CODE>aceDoc</CODE> an AceTimex object for each time expression.
     */
    ...
    private static boolean in(Object o, Object[] array) {
        for (int i = 0; i < array.length; i++) // if (array[i] == o) return true;
        {
    ...
    static {
        ...
        /* NameSubtyper.java: */ JetTest.validProperties.add("Word2Vec");
//        EventChecker = new SLEventChecker();
    }
