...
import static AceJet.EventPattern.containingSentence;
import java.util.*;
import java.io.*;
import Jet.Tipster.*;
import Jet.Parser.*;
...
import sleventextraction.SLEntity;
import sleventextraction.SLEventExtraction;
import static sleventextraction.SLEventExtraction.overlap;
import sleventextraction.SLEventTypeClassifier;
import sleventextraction.SLICMaxent;
import sleventextraction.SLRegularization;
 ...
 * <p>
 * One approach is based on creating a pattern from each event mention in the
 * training corpus. The pattern encompasses the event trigger and all of its
 * arguments, recording the connection between the trigger and the arguments
 * either as a sequence of chunks or as a path in a syntax graph. At decoding
 * time, a partial match of this pattern against the input can trigger an event.
 * <p>
 * The other approach is based on a set of maximum entropy classifiers. An
 * argument classifier decides whether a given entity mention is an argument of
 * a trigger, and a separate role classifier decides which role this argument
 * takes. After all potential arguments have been classified, an event
 * classifier decides whether, based on the arguments found, an event should be
 * triggered.
 */
public class EventTagger {
    ...
    static double EVENT_PROBABILITY_THRESHOLD = 0.20;
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            for (int j = 0; j < mentions.size(); j++) {
                ...
                for (String patternType : patternTypeList) {
                    ...
                    if (patternType.equalsIgnoreCase("CHUNK")) {
                        trainArgClassifier(event, m, doc, aceDoc, relations, ep);
                    }
                }
            }
    ...
    /**
     * trains two statistical models for event arguments: - argModel to decide
     * whether a mention is an argument of an event - roleModel to decide which
     * role the mention should have
     */
    private void trainArgClassifier(AceEvent event, AceEventMention eventMention,
            Document doc, AceDocument aceDoc, SyntacticRelationSet relations, EventPattern ep) {
        // get anchor
        ...
        /**
         * Added by Lei Sha 2015-4-30
         */
//        edu.stanford.nlp.pipeline.Annotation parsedsen = SLEventTypeClassifier.ParseSentence(doc.normalizedText(sentence));
//        List<CoreMap> sentences = ParseSentence.get(CoreAnnotations.SentencesAnnotation.class);
...
//        List<SLEntity> NPlist = SLEventExtraction.BPGetNPs(dependencies);
        /**
         * Add end
         */
        ...
        for (int im = 0; im < mentions.size(); im++) {
            ...
            for (int j = 0; j < ep.nodes.size(); j++) {
                AcePatternNode node = (AcePatternNode) ep.nodes.get(j);
                if (node != null && node.match(mention) > 0.1) {
                    maxmatch = 1;
                    break;
                }
            }

            Datum d = argumentFeatures(doc, anchor, event, mention, anchorMention, relations, maxmatch);
            //   outcome = argument role
            if ("noArg".equals(role)) {
                d.setOutcome("noArg");
                argFeatureWriter.println(d.toString());
                TrainEventTagger.SCCRFTag.RecordFeatureToFile("noArg", d, mention);
            } else {
                d.setOutcome("arg");
                argFeatureWriter.println(d.toString());
                TrainEventTagger.SCCRFTag.RecordFeatureToFile("arg", d, mention);
                d.setOutcome(role);
                roleFeatureWriter.println(d.toString());
            }
        }
        if (TrainEventTagger.SCCRFTag.GetBufferSize() > 1 && TrainEventTagger.SCCRFTag.posnum > 0) {
            TrainEventTagger.SCCRFTag.ClearBuffer(event);
        }
        TrainEventTagger.SCCRFTag.ClearBufferNotPrint();
    }

    public static Datum argumentFeatures(Document doc, String anchor, AceEvent event, AceMention mention,
            AceMention anchorMention, SyntacticRelationSet relations, int match) {
        String direction;
        ChunkPath cpath;
        ...
        d.addFV("evTypemenType", event.subtype + ":" + mention.getType());
        ...
        d.addFV("Match", String.valueOf(match));
        return d;
    }
    ...
    public static GISModel loadClassifierModel(String modelFileName) {
        try {
    ...
    private void collectArguments(AceEvent event, AceEventMention eventMention,
        ...
        for (int im = 0; im < mentions.size(); im++) {
            ...
            Datum d = argumentFeatures(doc, anchor, event, mention, anchorMention, relations, 0);
            //   classify:
    ...
    public void evaluatePatterns(Document doc, AceDocument aceDoc, String docId) {
        ...
        for (int i = 0; i < constituents.size(); i++) {
            ...
            if ("n".equals(cat) || "v".equals(cat) || "tv".equals(cat) || "ven".equals(cat)
                    || "ving".equals(cat) || "adj".equals(cat)) {
    ...
    void trainEventModel(Document doc, AceDocument aceDoc, String docId) {
        ...
        for (int i = 0; i < constituents.size(); i++) {
            ...
            if ("n".equals(cat) || "v".equals(cat) || "tv".equals(cat) || "ven".equals(cat)
                ...
                CONFIDENT_ARG = 0.10;
                if (anchor.contains("transport")) {
                    int a = 0;
                    a = a + 1;
                }
                AceEvent event = eventAnchoredByConstituent(constit, doc, aceDoc, docId, relations, 0);
                ...
                Datum d = eventFeatures(doc, anchor, constit, event, pattern, relations);
     ...
     */
    private Datum eventFeatures(Document doc, String anchor, Annotation anchorAnn,
            AceEvent event, EventPattern pattern, SyntacticRelationSet relations) {
        Datum d = new Datum();
        if (anchor.contains("/")) {
            d.addF(anchor.split("/")[0]);
        } else {
            d.addF(anchor);
        }
        if (pattern == null) {
        ...
        d.addFV("eventtype", event.type);
        d.addFV("eventsubtype", event.subtype);
        String pos = (String) anchorAnn.get("cat");
        if (!pos.equals("n") && !pos.equals("adj")) {
            pos = "v";
        }
        d.addFV("triggertype", pos);
        return d;
    }
    ...
    void trainCorefModel(Document doc, AceDocument aceDoc, String docId) {
        ...
        for (int i = 0; i < constituents.size(); i++) {
            ...
            if ("n".equals(cat) || "v".equals(cat) || "tv".equals(cat) || "ven".equals(cat)
                ...
                String anchor = EventPattern.normalizedAnchor(constit, doc, relations);
                Span anchorExtent = constit.span();
                AceEvent event = eventAnchoredByConstituent(constit, doc, aceDoc, docId, relations, aceEventNo);
    ...
    public void tag(Document doc, AceDocument aceDoc, String currentDocPath, String docId) {
        ...
        HashSet<Span> matchedAnchors = new HashSet<>();
...
//                    }
                    if (doc.normalizedText(constit).contains("detain")) {
                        int a = 0;
                        a = a + 1;
                    }
                    AceEvent event = eventAnchoredByConstituent(constit, doc, aceDoc, docId, relations, aceEventNo);
...
//                    AceEvent pruned
                    event = pruneEvent(event, constit, doc, relations);//maxent
                    if (matchedAnchors.contains(anchorExtent)) {
                        continue; //<< LeiSha added 14 May 2015
                    }
                    if (event != null) {
    ...
    AceEvent eventAnchoredByConstituent(Annotation constit, Document doc,
        ...
        String nowanchor = doc.normalizedText(constit);
        String anchor = EventPattern.normalizedAnchor(constit, doc, relations);
        Span anchorExtent = constit.span();
...
//        }
        AceEvent bestEvent
                = matchPatternSet(patterns, anchorExtent, anchor, doc, relations, aceDoc, nowanchor);
        if (bestEvent == null) {
            return null;
        }
        ...
        try {
            if (bestEvent.id.equals("classified")) {
                eventId += "classified";
            }
    ...
    /**
     * applies maxent event model and removes doubtful events and event
     * arguments. An event is removed if its probability (based on the event
     * model) is below EVENT_PROBABILITY_THRESHOLD and it does not have any
     * arguments assigned by patterns. An argument is removed if (probability of
     * event) * (probability of argument) is below
    ...
    AceEvent pruneEvent(AceEvent event, Annotation constit, Document doc,
        ...
        if (eventProb < EVENT_PROBABILITY_THRESHOLD) {
            ...
            if (event.id.contains("classified") && eventProb < 0.6) {
                return null;
            }
...
//        }
    ...
    AceEvent matchPatternSet(List patterns, Span anchorExtent, String anchor,
            Document doc, SyntacticRelationSet relations, AceDocument aceDoc, String nowanchor) {
        patternMatched = null;
         ...
         */
        if (true) {
            Annotation sen = containingSentence(doc, anchorExtent);
            if (bestEvent == null) {
                if (Ace.Testing) {
                    bestEvent = Ace.EventTypeClassifier.GetDirectAceEvent(anchor, doc.normalizedText(sen));
                } else {
                    bestEvent = TrainEventTagger.EventTypeClassifier.GetDirectAceEvent(anchor, doc.normalizedText(sen));
                }
            }
            if (bestEvent != null) {
                ArrayList mentions = aceDoc.getAllMentions();
                AceEventAnchor anchorMention = new AceEventAnchor(anchorExtent, anchorExtent, doc.text(anchorExtent).trim(), doc);
                ArrayList<AceEventMentionArgument> matchmentions;
                if (bestEvent.mentions.isEmpty()) {
                    matchmentions = new ArrayList<>();
                } else {
                    matchmentions = bestEvent.mentions.get(0).arguments;
                }
                bestEvent.mentions.clear();
                bestEvent.mentions.add(new AceEventMention("id", sen.span(), anchorExtent, doc.text()));

                for (int i = 0; i < mentions.size(); i++) {
                    AceMention m = (AceMention) mentions.get(i);
                    if (m.getJetHead().within(sen.span())) {
                        int maxmatch = 0;
                        if (patternMatched != null) {
                            for (int j = 0; j < matchmentions.size(); j++) {
                                if (overlap(matchmentions.get(j).value.text, m.text) > 0) {
                                    maxmatch = 1;
                                    break;
                                }
                            }
                        }
                        Datum d = EventTagger.argumentFeatures(doc, anchor, bestEvent, m, anchorMention, relations, maxmatch);
                        double argProb = argModel.eval(d.toArray())[argModel.getIndex("arg")];
                        String role = roleModel.getBestOutcome(roleModel.eval(d.toArray())).intern();
                        double roleProb = roleModel.eval(d.toArray())[roleModel.getIndex(role)];
                        m.argProb = argProb;
                        m.role = role;
                        double roleprob[] = GetBest5(roleModel, d);
                        System.arraycopy(roleprob, 0, m.roleProb, 0, roleprob.length);
                        Ace.SCCRFTestRecorder.RecordFeatureToFile("IDK", d, m);

                        if (argProb > 0.1) {
                            AceEventMentionArgument aema = new AceEventMentionArgument(m, role);
                            aema.roleConfidence = roleProb;
                            aema.confidence = argProb;
                            int hassame = -1;
                            for (int j = 0; j < bestEvent.mentions.get(0).arguments.size(); j++) {
                                if (bestEvent.mentions.get(0).arguments.get(j).value.text.contains(aema.value.text)) {
                                    hassame = 1;
                                } else if (aema.value.text.contains(bestEvent.mentions.get(0).arguments.get(j).value.text)) {
                                    hassame = 2;
                                    bestEvent.mentions.get(0).arguments.remove(j);
                                    j--;
                                }
                            }
                            if (hassame != 1) {
                                bestEvent.mentions.get(0).arguments.add(aema);
                                AceEventArgumentValue entity = aema.value.getParent();
                                bestEvent.arguments.add(new AceEventArgument(entity, role));
                            }
                        }
                    }
                }
                /**
                 * My addition for regularization
                 */
                if (Ace.Testing) {
                    int senpos = SLEventExtraction.GetsenPos(doc.normalizedText(sen));
                    if (senpos == -1) {
                        SLEventExtraction.GetsenPos(doc.normalizedText(sen));
                        return null;
                    }
                    CoreMap senCM = SLEventExtraction.sentenceCoreMaps.get(senpos);
                    SemanticGraph senSG = SLEventExtraction.sentenceSemanticGraphs.get(senpos);
                    IndexedWord iwanchor = SLEventTypeClassifier.GetCorrespondingIndexedWord(anchor, senCM);
                    if (iwanchor != null && bestEvent.mentions.get(0).arguments.size() > 1 && !bestEvent.id.contains("classified")) {
                        bestEvent.mentions.get(0).arguments.clear();
                        if (Ace.SCCRFTestRecorder.GetBufferSize() > 1) {
                            List<SLEntity> ModiEntities = new LinkedList<>();
                            for (AceMention m : Ace.SCCRFTestRecorder.EntityBuffer) {
                                SLEntity sle;
                                try {
                                    sle = new SLEntity(m, senCM, senSG);
                                } catch (java.lang.IndexOutOfBoundsException err) {
                                    continue;
                                }
                                ModiEntities.add(sle);
                            }
                            try {
                                SLEventExtraction.Lookup_All_wordvec(ModiEntities);
                            } catch (java.lang.NullPointerException err) {

                            }
                            double[][] Connectivity = new double[ModiEntities.size()][ModiEntities.size()];
                            for (int i = 0; i < ModiEntities.size(); i++) {
                                for (int j = i + 1; j < ModiEntities.size(); j++) {
                                    Datum d = Ace.regulator.ConnectiveFeatures(iwanchor, senCM, senSG, ModiEntities.get(i), ModiEntities.get(j), anchor);

                                    if (d == null) {
                                        continue;
                                    }
                                    d = SLICMaxent.DatumConverter(d);
                                    double pc = Ace.regulator.ConnectivityModel.eval(d.toArray())[Ace.regulator.ConnectivityModel.getIndex("yes")];
                                    Connectivity[i][j] = pc > 0.80 ? 1 : 0;
                                    Connectivity[i][j] = pc < 0.20 ? -1 : 0;
                                    Connectivity[j][i] = Connectivity[i][j];

                                }
                            }
                            double[] Probs = new double[ModiEntities.size()];
                            for (int i = 0; i < Probs.length; i++) {
                                Probs[i] = ModiEntities.get(i).argProb;
                            }
                            int[][] rt = new int[ModiEntities.size()][5];
                            double[][] rp = new double[ModiEntities.size()][5];
                            for (int i = 0; i < ModiEntities.size(); i++) {
                                double[] indexprob = ModiEntities.get(i).roleProb;
                                for (int j = 0; j < 5; j++) {
                                    if (indexprob[2 * j + 1] < 0.0001) {
                                        rt[i][j] = -1;
                                        rp[i][j] = -1;
                                    } else {
                                        rt[i][j] = (int) indexprob[2 * j];
                                        rp[i][j] = indexprob[2 * j + 1];
                                    }
                                }
                            }

                            SLRegularization.SLOptimize optim;
                            optim = new SLRegularization.SLOptimize(ModiEntities.size(), Connectivity, Probs, bestEvent, rt, rp);
                            int[] assignment = optim.BeamSearch();
                            for (int i = 0; i < assignment.length; i++) {
                                if (assignment[i] != 0) {
                                    Datum d = Ace.SCCRFTestRecorder.DataBuffer.get(i);
                                    String role = SLVectorModel.Roles[assignment[i] - 1];
//                                String role = ModiEntities.get(i).role;
                                    double roleProb = roleModel.eval(d.toArray())[roleModel.getIndex(role)];
                                    AceMention m = Ace.SCCRFTestRecorder.EntityBuffer.get(i);
                                    AceEventMentionArgument aema = new AceEventMentionArgument(m, role);
                                    aema.roleConfidence = roleProb;
                                    aema.confidence = m.argProb;
                                    bestEvent.mentions.get(0).arguments.add(aema);
                                    AceEventArgumentValue entity = aema.value.getParent();
                                    bestEvent.arguments.add(new AceEventArgument(entity, role));
                                }
                            }
                        } else if (Ace.SCCRFTestRecorder.GetBufferSize() == 1) {
                            Datum d = Ace.SCCRFTestRecorder.DataBuffer.get(0);
                            double argProb = argModel.eval(d.toArray())[argModel.getIndex("arg")];
                            String role = roleModel.getBestOutcome(roleModel.eval(d.toArray())).intern();
                            double roleProb = roleModel.eval(d.toArray())[roleModel.getIndex(role)];
                            if (argProb > 0.1) {
                                AceMention m = Ace.SCCRFTestRecorder.EntityBuffer.get(0);
                                AceEventMentionArgument aema = new AceEventMentionArgument(m, role);
                                aema.roleConfidence = roleProb;
                                aema.confidence = argProb;
                                bestEvent.mentions.get(0).arguments.add(aema);
                ...
                if (Ace.SCCRFTestRecorder.GetBufferSize() > 1 && Ace.SCCRFTestRecorder.posnum > 2) {
                    Ace.SCCRFTestRecorder.ClearBuffer(bestEvent);
                }
                Ace.SCCRFTestRecorder.ClearBufferNotPrint();
            }
        }
        /**
         * my addition end
...
//            logger.trace("event generation score  = {}", bestPattern.evaluation.test(bestEvent.arguments, 0.50));
    ...
    /**
     * tag a set of documents with ACE event information, using entity and value
     * information from an existing APF file, typically the key. Takes the
     * following arguments:
     * <ul>
     ...
     * <li> outDir: directory to receive updated APF files including event
     * information
    ...
    private double[] GetBest5(GISModel roleModel, Datum d) {
        double[] res = new double[10];
        double[] eval = roleModel.eval(d.toArray());
        PriorityQueue<Pair<Integer, Double>> pq = new PriorityQueue<>(5, new Comparator() {
        ...
        for (int i = 0; i < eval.length; i++) {
            pq.add(new Pair<>(i, eval[i]));
        }
        for (int i = 0; i < 10; i += 2) {
            Pair<Integer, Double> poll = pq.poll();
            String role = roleModel.getOutcome(poll.first());
            int pos = SLVectorModel.CheckPos(role, SLVectorModel.Roles);
            if (pos == -1) {
                break;
            }
//            res[pos] = poll.getValue();
            res[i] = pos;
            res[i + 1] = poll.second();
        }
        return res;
