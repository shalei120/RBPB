...
import static AceJet.EventTagger.argModel;
import static AceJet.EventTagger.roleModel;
import java.util.*;
import Jet.Tipster.*;
import Jet.Parser.*;
...
public class EventPattern {
    ...
    EventPattern(String patternType, Document doc, SyntacticRelationSet relations,
        ...
        if ("CHUNK".equals(patternType)) {
            paths = new ArrayList<>();
        } else if ("SYNTAX".equals(patternType) || "PA".equals(patternType)) {
        ...
        for (int iarg = 0; iarg < argVals.size(); iarg++) {
            ...
            if (iarg > 0 && "CHUNK".equals(patternType)) {
        ...
        if ("SYNTAX".equals(patternType) || "PA".equals(patternType)) {
    ...
    /**
     * returns true if this is a pattern which can never match: a syntax or PA
     * pattern with no syntactic relations.
     */
    public boolean empty() {
        return ("SYNTAX".equals(patternType) || "PA".equals(patternType))
    ...
    static String normalizedAnchor(Span anchorSpan, String anchorText, Document doc,
        ...
        if (constits != null) {
            for (int i = 0; i < constits.size(); i++) {
                ...
                if ("n".equals(cat) || "v".equals(cat) || "tv".equals(cat) || "ven".equals(cat) || "ving".equals(cat)
                        || "adj".equals(cat)) {
    ...
    /**
     * match an anchor and its context against the event patterns; if the match
     * is successful, build and return an AceEvent.
     *
     * @param anchorExtent
     * @param anchor
     * @param doc
    ...
    public AceEvent match(Span anchorExtent, String anchor, Document doc,
        ...
        for (int iarg = 0; iarg < argumentValue.size(); iarg++) {
            ...
            int hassame = -1;
            for (int i = 0; i < emention.arguments.size(); i++) {
                if (emention.arguments.get(i).value.text.contains(marg.value.text)) {
                    hassame = 1;
                } else if (marg.value.text.contains(emention.arguments.get(i).value.text)) {
                    hassame = 2;
                    emention.arguments.remove(i);
                    i--;
                }
            }
            if (hassame != 1) {
                addEventMentionArgument(emention, marg.role, marg.value, marg.confidence);
            }

            hassame = -1;
            for (int i = 0; i < event.arguments.size(); i++) {
                if (event.arguments.get(i).role.equalsIgnoreCase(marg.role)) {
                    hassame = 1;
                    break;
                }
            }
    ...
    private void addEventMentionArgument(AceEventMention emention, String role, AceMention mention, double confidence) {
        ...
        AceEventMentionArgument aema = new AceEventMentionArgument(mention, role);
        aema.confidence = confidence;
        emention.addArgument(aema);
    }
    ...
    /**
     * matches the chunk pattern portion of this event pattern against Document
     * 'doc' anchored at 'anchorExtent'. Returns a list of
    ...
    private ArrayList chunkMatch(Span anchorExtent, Document doc,
        ...
        for (int inode = anchorPosn - 1; inode >= 0; inode--) {
            ...
            while (!mentionsBeforeTrigger.isEmpty()) {
                ...
                if ((nodeScore = node.match(m)) > MIN_MATCH_SCORE && AceEventArgument.isValid(eventSubtype, role0, m) && new ChunkPath(doc, m, lastM).equals(path)) {
//                     match
        ...
        for (int inode = anchorPosn; inode < nodes.size() - 1; inode++) {
            ...
            while (!mentionsAfterTrigger.isEmpty()) {
                ...
                if ((nodeScore = node.match(m)) > MIN_MATCH_SCORE && AceEventArgument.isValid(eventSubtype, role0, m) && new ChunkPath(doc, lastM, m).equals(path)) {
//                     match
                    nodeScore = node.match(m);
                    chunkMatchScore += nodeScore;
    ...
    public static Annotation containingSentence(Document doc, Span span) {
    ...
    /**
     * computes the extent of an event: the span from the leftmost argument (or
     * anchor) to the rightmost argument (or anchor).
     */
    public static Span computeExtent(Span anchorExtent, ArrayList argumentValue) {
    ...
    /**
     * write the event pattern to <CODE>w</CODE> in a form which can easily be
     * reloaded
     */
