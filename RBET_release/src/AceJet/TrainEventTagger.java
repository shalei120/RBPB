...
public class TrainEventTagger {
    ...
    static SLMentionRecorder SCCRFTag;
    static SLRegularization regulator;
    static SLEventTypeClassifier EventTypeClassifier;

    /**
     * train an ACE event tagger from a set of documents with APF files. Takes
     ...
     * <li> outDir: directory to receive all temporary and output files (event
     * pattern file, event and argument models, etc.)
    ...
    public static void main(String[] args) throws IOException {
        ...
        regulator.ConnectivityFeatureWriter.close();                            // Added by Lei Sha 2015-5-11
        regulator.trainMaxEnt();
...
//        trainCorefModel(et);
    ...
    /**
     * train the event model -- the classifier which decides whether to actually
     * report an event.
     * <p>
     * The event model is supposed to help in the case of gaps in the pattern
     * sets, so the training is a bit complicated. We build a pattern set and
     * arg and role models from half the training corpus (the odd documents),
     * and then apply these models to the other half of the training corpus (the
     * even documents), and then train the event model using the results of
     * these models combined with the true information about the presence of
     * events.
     */
     ...
     *
     * @param fileList a list of text file names, one per line. The APF file
     * names are obtained by replacing 'sgm' by 'apf.xml'.
     */
    public static void train(EventTagger et, String fileList, int pass) throws IOException {
        ...
        while ((currentDocPath = reader.readLine()) != null) {
            Ace.currentDocPath = currentDocPath;
//            Ace.EntityConstructor.Construct_by_doc(currentDocPath+".sgm");
            docCount++;
...
//            SLEventExtraction.parseFile(textFile);
            doc.setAllTags(true);
            doc.open();
            doc.stretchAll();
            Resolve.ACE = true;
    ...
    static public void buildClassifierModel(String featureFileName, String modelFileName) {
    ...
    static {
        SCCRFTag = new SLMentionRecorder(1);
        regulator = new SLRegularization(1);
    }
}
