...
public class EventSyntacticPattern {
     ...
     *
     * todo: set of {offset in document} of nodes to be processed indexMap:
     * offset in document --> index in syntactic pattern (+n for n-th argument,
     * -n for n-th variable node) wordMap: offset in document --> word
     * (normalized form) path: offset in document --> set of syntactic relations
     * which reach this path from anchor
     */
    static SyntacticRelationSet buildPattern(String patternType,
        ...
        while (todo.size() > 0 && argsToConnect > 0) {
            ...
            for (int ifrom = 0; ifrom < fromSet.size(); ifrom++) {
                ...
                if (patternType == "SYNTAX" && isPaRelation(r)) {
                    continue;
                }
                ...
                SyntacticRelation rp = new SyntacticRelation(fromIndex, r.sourceWord, r.type, indexMap.get(to), r.targetWord);
     ...
     */
    static String buildSyntacticPath(int anchorStart, int argumentPosn, SyntacticRelationSet relations) {
    ...
    /**
     * matches the syntactic pattern 'syntax' of this EventPattern against the
     * syntactic relations 'relations' of the current sentence. If a subgraph of
     * 'syntax' rooted at the anchor matches a subgraph of 'relations', returns
     * a list of the matching arguments (a list of AceEventMentionArguments).
     * <p>
     * To keep track of the correspondence between the two graphs, uses
     *
     * offsetMap: index in pattern --> list of offsets in sentence [normally a
     * one-element list, but there may be more than one for a conjoined
     * structure]
     *
     * toDo: list of nodes in 'syntax' yet to be processed (initialized to
     * anchor node and then expanding outward in a breadth-first search)
     */
