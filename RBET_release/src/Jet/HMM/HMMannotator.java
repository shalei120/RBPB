...
public class HMMannotator {
     ...
     * tags associated with the states and the annotations on the documents. The
     * tag table is a two-dimensional array, where each row contains 4 elements:
     * <br>
     * annotation-type | feature [or null if no feature constraint] |
     * feature-value | tag <br>
     * This specifies that the annotation with the given annotation-type (and
     * feature / feature-value, if not null) matches the HMM state with tag
     * 'tag'.
     */
    ...
    /**
     * read the tag table (the list of annotation types and features) from file
     * 'tagFileName'.
     */
    ...
    /**
     * read the tag table (the list of annotation types and features) from
     * BufferedReader 'in'. Each line must be of the form <br>
     * annotationType HMMtag <br>
     * or annotationType feature featureValue HMMtag <br>
     * where 'HMMtag' ties this line to a state of the HMM. Stops at the
     * end-of-file or on encountering a line 'endtags'.
     */
    ...
    /**
     * writes the tag table (the correspondence between HMM tags and annotation
     * types and features) to PrintWriter 'pw'.
     */
    ...
    /**
     * returns the tag table (the correspondence between HMM tags and annotation
     * types and features).
     */
     ...
     * the tag table is matched directly against the tag on the state, If the
     * flag is true, the tag in the tag table matches tags B-tag and I-tag on
     * HMM states. More precisely, if an annotation is associated with a tag X
     * through the tag table, and the annotation spans three tokens, the first
     * token must match an HMM state with tag B-X, and the remaining tokens must
     * match HMM states with tag I-X.
     */
    ...
    /**
     * sets / clears the annotateEachToken flag, which applies only if BItag ==
     * false. If annotateEachToken is true, then the span associated with each
     * token receives a separate annotation (if assigned a tag which corresponds
     * to an annotation), If this flag is false, then we look for the maximal
     * sequence of tokens which are assigned the same tag, and assign it a
     * single annotation (again, if the tag corresponds to an annotation).
     */
    ...
    /**
     * sets the zones to be annotated. For example, if zone="P", then each
     * sequence of tokens spanned by an annotation of type P will be processed,
     * either to train the HMM or to annotate the text using the HMM.
     */
     ...
     * annotation and the log probability of the most likely analysis excluding
     * this annotation.
     */
    ...
    /**
     * turn on/off the feature that records the (log) probability of the
     * assignment of tags to a span of text. It is recorded as an annotation
     * <b>HMMtags</b> with a feature <b>prob</b>.
     */
    ...
    /**
     * limit on length (in tokens) of an annotation; longer annotations are not
     ...
     * document has 'zoneToTag' annotations, and trains on each zone separately.
     */
    ...
    public void trainOnSpan(Document doc, Span textSpan) {
        ...
        while (posn < end) {
            ...
            for (int i = 0; i < anns.size(); i++) {
                ...
                if ((tag = tagForAnnotation(ann)) != null) {
                    break;
                }
            }
            if (tag != null) {
    ...
    /**
     * use the annotations on all documents in DocumentCollection 'col' to train
     * HMM 'h'.
     */
    ...
    /**
     * use the HMM to add annotations to Document 'doc'. The Viterbi algorithm
     * is used to find the most likely state sequence for each token sequence;
     * the tags on the resulting states are used to generate annotations (based
     * on the tagTable).
     */
     ...
     * likely state sequences are computed for each token sequence; the tags on
     * the resulting states are used to generate annotations (based on the
     * tagTable). The set of annotations associated with each sequence is tagged
     * as a separate hypothesis with <b>hypo</b> tag <CODE>hypId</CODE> followed
     * by the hypothesis number.
     */
    ...
    /**
     * use the HMM to add annotations to Span 'textSpan' of Document 'doc'. The
     * Viterbi algorithm is used to find the most likely state sequence for each
     * token sequence; the tags on the resulting states are used to generate
     * annotations (based on the tagTable).
     */
    ...
    /**
     * use the HMM to add annotations to Span 'textSpan' of Document 'doc'. The
     * <CODE>n</CODE> most likely state sequences are computed for each token
     * sequence. The tags on the resulting states are used to generate
     * annotations (based on the tagTable). The annotations for the <i>i</i>th
     * most likely state sequence are marked as a hypothesis with <b>hypo</b>
     * feature <CODE>hypId</CODE>-<i>i</i>.
     *
